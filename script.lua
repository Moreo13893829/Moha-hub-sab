...
local isexecutorclosure = rawget(_G, "isexecutorclosure") or function(...) return false end
local newcclosure = rawget(_G, "newcclosure") or function(f) return f end
local isfile = rawget(_G, "isfile") or function(...) return false end
local readfile = rawget(_G, "readfile") or function(...) return "" end
local writefile = rawget(_G, "writefile") or function(...) end
local firesignal = rawget(_G, "firesignal") or function(...) end
local cloneref = rawget(_G, "cloneref") or function(r) return r end
local fireproximityprompt = rawget(_G, "fireproximityprompt") or function(...) end
local getconnections = rawget(_G, "getconnections") or function(...) return {} end
local request = rawget(_G, "request") or rawget(_G, "http_request") or function(...) end
local raknet = rawget(_G, "raknet") or {}
...
local function fireProximityPromptSafely(prompt)
    if not prompt or not prompt.Parent then return end
    local char = LocalPlayer.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end
    
    local targetPart = prompt.Parent:IsA("Attachment") and prompt.Parent.Parent or prompt.Parent
    ...
    if currentDist > maxDist then
        local targetPos = targetPart.Position + (hrp.Position - targetPart.Position).Unit * (maxDist - 1.5)
        local targetCFrame = CFrame.new(targetPos, targetPos + hrp.CFrame.LookVector)
        
        local tweenInfo = TweenInfo.new(
            math.clamp(currentDist / 40, 0.1, 0.8),
...
local FileName = "DexHub_Config.json" 
local DefaultConfig = {
    Positions = {
...
local function triggerClosestUnlock(yLevel, maxY)
    ...
    if targetPrompt then
        if fireproximityprompt then
            fireProximityPromptSafely(targetPrompt)
        else
...
function applyTheme(themeName)
    ...
    task.spawn(function()
        ...
        if buildDexSettingsUI then
            buildDexSettingsUI()
        end
        task.wait()
        if _G.DexSettingsUI then
            if _G.DexSettingsUI.switchTab then
                _G.DexSettingsUI.switchTab(savedTab)
            end
            if wasVis and _G.DexSettingsUI.panel then
...
MakeDraggable = function(handle, target, saveKey)
...
ShowNotification = function(title, text) end
...
if Config.UltraLightMode then
    task.spawn(function()
        ...
        for _,gn in ipairs({"AutoStealUI","DexAdminPanel","SettingsUI","StealSpeedUI",
            "DexInvisPanel","DexStatusHUD","DexSettings","DexPlatformUI",
            "DexAutoBuyUI","DexMiniActions"}) do
            local sg = PlayerGui:FindFirstChild(gn)
            if sg then
                for _,obj in ipairs(sg:GetDescendants()) do
                    if obj.Name == "RacetrackBorder" and obj:IsA("UIStroke") then
...
local _carpetSpeedConnection = Connections.carpetSpeedConnection
...
local function setCarpetSpeed(enabled)
    ...
    _carpetSpeedConnection = Connections.carpetSpeedConnection
...
local _infiniteJumpConnection = Connections.infiniteJumpConnection
...
local function setInfiniteJump(enabled)
    ...
    _infiniteJumpConnection = Connections.infiniteJumpConnection
    if not enabled then return end
...
local _antiRagdollConn = Connections.antiRagdollConn
...
local function stopAntiRagdoll()
    ...
    _antiRagdollConn = Connections.antiRagdollConn
end
...
task.spawn(function()
    ...
    local function executeInstantSteal(prompt)
        ...
        pcall(function()
            if fireproximityprompt then
                fireProximityPromptSafely(prompt)
            end
        end)
    ...
    ShowPriorityAlert = function(brainrotName, genText, mutation, ownerUsername)
        ...
        local existing = PlayerGui:FindFirstChild("DexPriorityAlert")
        if existing then existing:Destroy() end
        
        local alertGui = Instance.new("ScreenGui")
        alertGui.Name = "DexPriorityAlert"
...
local     canUseAdminAction = function(targetPlayer)
    ...
    updateProximityAPButton = function()
...
-- isOnCooldown forward declaration syntax error removed (already declared on line 16)
...
local _isTpMoving = State.isTpMoving
...
local function runAutoSnipe()
    ...
    local _cloner = LocalPlayer.Backpack:FindFirstChild("Quantum _cloner") or char:FindFirstChild("Quantum _cloner")
...
if IS_MOBILE then
    ...
    mobileGui.Name = "DexMobileControls"
    ...
    createMobBtn("UI", Color3.fromRGB(255, 255, 255), 5, function()
        ...
        local adUI = PlayerGui:FindFirstChild("DexAdminPanel")
...
CreateToggleSwitch(rShowUnlockHUD, Config.ShowUnlockButtonsHUD, function(ns, set)
    ...
    local hudGui = PlayerGui:FindFirstChild("DexStatusHUD")
    if hudGui then
...
local arV1SetRef, arV2SetRef = {fn = nil}, {fn = nil}
...
local nearestToggleRef = {setFn = nil}
local highestToggleRef = {setFn = nil}
local priorityToggleRef = {setFn = nil}
...
CreateToggleSwitch(rJoinerRow, Config.ShowJobJoiner, function(ns, set)
    ...
    local gui = PlayerGui:FindFirstChild("DexJobJoiner")
...
bReset.MouseButton1Click:Connect(function()
    ...
    if PlayerGui:FindFirstChild("DexAdminPanel") and PlayerGui.XiAdminPanel:FindFirstChild("Frame") then
    ...
    if PlayerGui:FindFirstChild("DexInvisPanel") and PlayerGui.XiInvisPanel:FindFirstChild("Frame") then
...
if not IS_MOBILE then
    UserInputService.InputBegan:Connect(function(input, gp)
        if input.KeyCode == (Enum.KeyCode[Config.MenuKey] or Enum.KeyCode.LeftControl) then
            if _G.DexSettingsUI and _G.DexSettingsUI.panel then
                _G.DexSettingsUI.panel.Visible = not _G.DexSettingsUI.panel.Visible
            else
                settingsGui.Enabled = not settingsGui.Enabled
            end
        end
        ...
        if Config.JobJoinerKey and input.KeyCode == Enum.KeyCode[Config.JobJoinerKey] then
            local joinerGui = PlayerGui:FindFirstChild("DexJobJoiner")
            if joinerGui then
...
task.spawn(function()
    ...
    if Config.HideInvisPanel then
        local g = PlayerGui:FindFirstChild("DexInvisPanel")
        if g then g.Enabled = false end
    end
    if Config.HideStatusHUD then
        local g = PlayerGui:FindFirstChild("DexStatusHUD")
        if g then g.Enabled = false end
    end
    if Config.HideAutoBuyUI then
        local g = PlayerGui:FindFirstChild("DexAutoBuyUI")
        if g then local p = g:FindFirstChild("ABPanel"); if p then p.Visible = false end end
    end
    if Config.HidePlatformUI then
        local g = PlayerGui:FindFirstChild("DexPlatformUI")
        if g then g.Enabled = false end
    end
...
task.spawn(function()
    ...
    brainrotESPFolder.Name = "DexBrainrotESP"
...
task.spawn(function()
	...
	local tracks = {}
	local _clone, oldRoot, _hip, connection, rootJoint, originalC0
	...
	local function doClone()
		...
		if character and character:FindFirstChild("Humanoid") and character.Humanoid.Health > 0 then
			...
			rootJoint = character:FindFirstChild("Root", true) or character:FindFirstChild("RootJoint", true)
			if not rootJoint then return false end
			_hip = character.Humanoid.HipHeight
			originalC0 = rootJoint.C0
			local sinkValue = _G.SinkSliderValue or 5
			local rotAngle = _G.InvisStealAngle or 180
			rootJoint.C0 = originalC0 * CFrame.new(0, -sinkValue, 0) * CFrame.Angles(math.rad(rotAngle), 0, 0)
			ghostEnabled = true
			return true
		end
		return false
...
task.spawn(function()
    ...
    local function buildHUD()
        ...
        authors.Text = "mohamed79109 & Lyubomyr2012"
...
task.spawn(function()
    local function makeBlacklistESP(player)
        ...
        local old = char:FindFirstChild("DexBlacklistESP")
        if old then old:Destroy() end

        local msg = Config.BlacklistMsg or "BLOCKED"

        local bb = Instance.new("BillboardGui")
        bb.Name                  = "DexBlacklistESP"
    ...
    local function removeBlacklistESP(player)
        local char = player.Character
        if char then
            local bb = char:FindFirstChild("DexBlacklistESP")
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            ...
            pcall(function()
                if isBlacklisted and isBlacklisted(p.Name) then
                    ...
                    if char and not char:FindFirstChild("DexBlacklistESP") then
                        makeBlacklistESP(p)
                    end
                else
                    removeBlacklistESP(p)
...
function buildDexSettingsUI()
    ...
    local oldBSG2 = pg:FindFirstChild("DexSettings")
    ...
    do
        local sp = Config.DexSettingsPos
        if sp then
    ...
    local function switchTab2(id)
        _activTab2 = id
        if _G.DexSettingsUI then _G.DexSettingsUI.currentTab = id end
    ...
    makeBtn(aS, "Job Joiner",              6, function()
        local g = PlayerGui:FindFirstChild("DexJobJoiner")
        if g then g.Enabled = not g.Enabled end
    end)
    ...
    makeToggle(vS,"Stealing HUD",function() return Config.ShowStealingHUD~=false end,function(v) Config.ShowStealingHUD=v; SaveConfig(); local g=PlayerGui:FindFirstChild("DexStealingHUD"); if g then g.Enabled=v end end,18)
    ...
    do
        ...
        ulBtn.MouseButton1Click:Connect(function()
            ...
            if ns then
                ...
                for _,gn in ipairs({"AutoStealUI","DexAdminPanel","SettingsUI","StealSpeedUI","DexInvisPanel","DexStatusHUD","DexSettings","DexPlatformUI","DexAutoBuyUI","DexMiniActions"}) do
    ...
    makeToggle(uhS,"Hide Admin Panel",function() return Config.HideAdminPanel end,function(v) Config.HideAdminPanel=v; SaveConfig(); local g=PlayerGui:FindFirstChild("DexAdminPanel"); if g then g.Enabled=not v end end,11)
    ...
    makeToggle(uhS,"Hide Auto Buy UI",function() return Config.HideAutoBuyUI end,function(v) Config.HideAutoBuyUI=v; SaveConfig(); local g=PlayerGui:FindFirstChild("DexAutoBuyUI"); if g then local p=g:FindFirstChild("ABPanel"); if p then p.Visible=not v end end end,13)
    ...
    makeToggle(uhS,"Hide Status HUD",function() return Config.HideStatusHUD end,function(v) Config.HideStatusHUD=v; SaveConfig(); local g=PlayerGui:FindFirstChild("DexStatusHUD"); if g then g.Enabled=not v end end,15)
    makeToggle(uhS,"Hide Invis Panel",function() return Config.HideInvisPanel end,function(v) Config.HideInvisPanel=v; SaveConfig(); local g=PlayerGui:FindFirstChild("DexInvisPanel"); if g then g.Enabled=not v end end,16)
    ...
    makeToggle(blS, "Show Blacklist ESP", function() return Config.BlacklistESP ~= false end, function(v)
        ...
        if not v then
            for _, p in ipairs(Players:GetPlayers()) do
                local char = p.Character
                if char then
                    local existing = char:FindFirstChild("DexBlacklistESP")
                    if existing then existing:Destroy() end
                end
            end
        end
...
function buildMiniActionsUI()
    local pg = PlayerGui
    if not pg then return end
    local oldG = pg:FindFirstChild("DexMiniActions")
    if oldG then oldG:Destroy() end

    local maGui = Instance.new("ScreenGui")
    maGui.Name = "DexMiniActions"
    ...
    local _oldAB = PlayerGui:FindFirstChild("DexAutoBuyUI")
    if _oldAB then _oldAB:Destroy() end

    local abGui = Instance.new("ScreenGui")
    abGui.Name         = "DexAutoBuyUI"
    ...
    task.spawn(function()
        ...
        local function firePurchaseNatural(prompt)
            ...
            pcall(function()
                if fireproximityprompt then fireProximityPromptSafely(prompt) end
            end)
...
task.spawn(function()
    ...
    if Config.AutoHideMiniUI then
        local g = PlayerGui:FindFirstChild("DexMiniActions")
...
local function setFloat(enabled) Config.FloatEnabled=enabled; SaveConfig(); if _G._floatConn then _G._floatConn:Disconnect(); _G._floatConn=nil end; if _G._floatBodyVelocity then pcall(function() _G._floatBodyVelocity:Destroy() end); _G._floatBodyVelocity=nil end; if not enabled then return end; local c=LocalPlayer.Character; local h=c and c:FindFirstChild("HumanoidRootPart"); if h then local bv=Instance.new("BodyVelocity"); bv.Name="FloatBodyVelocity"; bv.MaxForce=Vector3.new(0,math.huge,0); bv.Velocity=Vector3.new(0,30,0); bv.Parent=h; _G._floatBodyVelocity=bv; _G._floatConn=RunService.RenderStepped:Connect(function() if h and h.Parent then bv.Velocity=Vector3.new(0,30,0) end end) end end
...
task.spawn(function()
    local shGui=Instance.new("ScreenGui"); shGui.Name="DexStealingHUD"; shGui.ResetOnSpawn=false; shGui.Enabled=Config.ShowStealingHUD~=false; shGui.Parent=PlayerGui
...
task.spawn(function()
    local espF=Instance.new("Folder",Workspace); espF.Name="DexConvESP"; local cur=nil
